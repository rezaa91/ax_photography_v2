if (document.querySelector(".upload-wrapper")) {
    ...
    function toggleAlbumType(e) {
        ...
        if (target.checked) {
            switch (btnValue) {
                case "existing":
                    selectAlbumSection.hasAttribute("class", "hide") &&
                        selectAlbumSection.classList.remove("hide");
                    createAlbumSection.hasAttribute("class", "hide") &&
                        createAlbumSection.classList.add("hide");
                    break;

                case "new":
                    selectAlbumSection.hasAttribute("class", "hide") &&
                        selectAlbumSection.classList.add("hide");
                    createAlbumSection.hasAttribute("class", "hide") &&
                        createAlbumSection.classList.remove("hide");
                    break;
            }
        }
    }
    ...
    function setupEventListeners() {
        ...
        createAlbumInput.addEventListener(
            "input",
            setExistingAlbumValueToDefault
        );
        selectedExistingAlbum.addEventListener(
            "change",
            removeCreateAlbumValue
        );
